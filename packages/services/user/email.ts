...
function layout(title: string, body: string, actionUrl: string) {
    ...
    <div style="font-family:Inter,Arial,sans-serif;color:#18181b;line-height:1.6">
      <h1 style="font-size:22px;margin:0 0 16px">HexForm</h1>
...
export async function sendVerificationEmail(to: string, fullName: string, url: string) {
    const subject = "Verify your HexForm email";
    const text = [
        `Hi ${fullName},`,
        "Verify your HexForm email address using this link:",
        url,
    ...
    return sendEmail({
        ...
        html: layout(
            ...
            `<p>Hi ${escapeHtml(fullName)},</p><p>Confirm this email address to finish setting up your HexForm account.</p><p>${buttonHtml("Verify email", url)}</p>`,
            url,
        ),
    });
}
...
export async function sendPasswordResetEmail(to: string, fullName: string, url: string) {
    const subject = "Reset your HexForm password";
    const text = [
        `Hi ${fullName},`,
        "Reset your HexForm password using this link:",
        url,
    ...
    return sendEmail({
        ...
        html: layout(
            "Reset your password",
            `<p>Hi ${escapeHtml(fullName)},</p><p>Use this secure link to choose a new HexForm password.</p><p>${buttonHtml("Reset password", url)}</p>`,
            url,
        ),
    });
}
...
export async function sendPaymentReceiptEmail(payload: {
    ...
    const subject = "Your HexForm payment receipt";
    const bill = [
